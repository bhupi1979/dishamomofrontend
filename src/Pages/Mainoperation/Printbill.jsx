...
const billHeight = headerFooterHeight + (rowHeight * totalRows);
    ...
    doc.text("Price", 55, startY);
    doc.text("Sub-Total", 65, startY);
 ...
 obj.orders.forEach((item) => {
      ...
      doc.text(String(item.price), 55, y) // Bathrooms
      doc.text(String(item.total), 65, y) // Bathrooms
      y += 6;
    ...
    doc.setFontSize(10);
    doc.text(`Grand Total:Rs ${grandTotal.toFixed(2)}`, 35, y + 8)
